...
void exchangeHeadAndTail(Node *&head_ref) {
    ...
    // Buat kondisi jika hanya 2 node, cukup swap head\_ref
    if (head->next == tail && tail->prev == head) {
        // Swap head dan tail
        head_ref = tail;
        return;
    }
    // Simpan neighbor ( yaitu head\_next dan tail\_prev)
    Node* head_next = head->next;
    Node* tail_prev = tail->prev;

    // Update koneksi: tail\_prev \<-\> tail \<-\> head\_next
    tail->next = head_next;
    tail->prev = head;
    head_next->prev = tail;

    // Update koneksi: head\_next \<- ... \-\> tail\_prev \<-\> head \<-\> tail\_prev
    head->next = tail;
    head->prev = tail_prev;
    tail_prev->next = head;

    // terakhir Update head\_ref
    head_ref = tail;
