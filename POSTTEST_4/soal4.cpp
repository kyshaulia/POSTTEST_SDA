...
void sortedInsert(Node *&head_ref, int data) {
    ...
    if (data < head_ref->data) {
        ...
        // 1. Jika data < head\_ref-\>data, sisipkan sebelum head dan update head\_ref
    ...
    // 1\. Gunakan pointer current mulai dari head\_ref
    Node* current = head_ref;

    // 2\. Loop: while (current-\>next \!= head\_ref && current-\>next-\>data \< data)
